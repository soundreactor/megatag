...
function AddTags(obj, elem) {
  ...
  for (var i = 0; i < ojson.tag.length; i++) {
    ...
    cln.getElementsByTagName('span')[0].addEventListener("click", function(theVar) {
      return function() {
        DeleteTag(theVar)
      };
    }(cln), false);
    mainelem.insertBefore(cln, mainelem.childNodes[mainelem.childNodes.length - 1]);
  }
}
...
function aftergetlist() {
  ...
  var classname = document.getElementsByClassName("megatag-item-search");
  for (var i = 0; i < classname.length; i++) {
    classname[i]
      .addEventListener("keyup", function(event) {
        event.preventDefault();
        if (event.keyCode == 13) {
          ...
          var urll = this.getAttribute('data');
          var clbkloc = window.location = location.protocol + '//' + location.host + location.pathname;
          var cmdd = '?x={"action":"get_items","tag":"' + this.value + '","callback":"' + clbkloc + '"}';
          console.log(urll + cmdd);
          window.location = urll + cmdd;
        }
      });
  }
}
